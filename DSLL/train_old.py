...
def AL_train_DSLL_model(hyper_params, featureKD_model, train_X, train_Y, mapping_train_Y_new, train_Y_new, test_X, mapping_test_Y_new, test_Y_new, train_loader, test_ds, use_al, seednr):
    ...
    for altype in altypes:
        ...
        lpm_selection = "random" #"kmeans" #"kmeans"
        ...
        # how big is the active learning budget
        # make the budget the same or half as the max
        budget = int(max_budget-3) #int(max_budget/2)
        # for each epoch
        x_axis, ndcg_saved = [], []
        ...
        if lpm_selection == "kmeans":
            ...
            # scaled_features = scaler.fit_transform(xpool.numpy())
            # principalComponents = pca.fit_transform(scaled_features)
            # print(principalComponents.shape)
            # print(type(principalComponents))
            # print(xpool.numpy().shape)
            # print(type(data))
            principalComponents = umap.UMAP(metric='mahalanobis', n_neighbors=30, n_components=2).fit_transform(xpool.numpy())
            # principalComponents = 
        ...
        for epoch in range(hyper_params.classifier_epoch):
            ...
            # loop over the max iterations of teh budget
            for bud in range(budget):
                ...
                if altype == "random_cluster":
                    # chosen_indices = active_random(xseed, y_mappingseed, yseed, xpool, y_mappingpool, ypool, batch_size)
                    chosen_indices, chosen_clusters = [], []
                    # xpool.to(device), y_mappingpool.to(device)
                    # choose the items if the cluster is not already represented (chosen)
                    # randomly chose an index for cluster but if in cluster then not
                    while len(chosen_indices) < 10:
                        rand_index = np.random.randint(0, len(xpool), 1)
                        if df2.loc[rand_index] not in chosen_clusters:
                            chosen_clusters.append(df2.loc[rand_index])
                            chosen_indices.append(rand_index)

                        # df2.drop()
                        df2.drop(df2.loc[rand_index], inplace=True)
                    ...
                    if lpm_selection == "original":
                        ...
                        # we need to reset the index, the index of the df1 starts with 0 and the df2 is dropping indices in some places
