...
def split_label_pandas(data, past_label_num_ratio, hyper_params):   # full-label Y data, past_label_ratio
    # np.random.seed(25) # from 22: 11ij_6,      23: ij6_11 25:611_ij
    np.random.seed(int(hyper_params.split_seed))
    # array of shufled column names
    shuffled_indices = np.random.permutation(list(data))
...
def predict_integrated(model, input_X, input_mapping_Y_new):
    model.eval()
    if torch.cuda.is_available():
        output_Y, _, _ ,_ = model(Variable(torch.FloatTensor(input_X)).cuda(), Variable(torch.FloatTensor(input_mapping_Y_new)).cuda())
    else:
        output_Y, _, _ ,_ = model(Variable(torch.FloatTensor(input_X)), Variable(torch.FloatTensor(input_mapping_Y_new)))
    # print(output_Y)
    prediction = torch.sigmoid(output_Y).cpu().detach().numpy()
    model.train()
    return prediction
