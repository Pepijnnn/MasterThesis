...
def al_train_test_split(x, ymap, yold, ynew, test_percentage):
    ...
    # x_train, ymap_train, x_test, ymap_test = iterative_train_test_split(x, ymap, test_size = 0.2)

    # yold_train, ynew_train, yold_test, ynew_test = iterative_train_test_split(yold, ynew, test_size = 0.2)
...
def CustomActiveLearningDataset(x_tensor, y_mapping_tensor, old_y_tensor, y_tensor, batch_size = 10):
    # splits the dataset in train (seed and pool) and test set, not needed if test set already given
    test_percentage = 10
...
if __name__ == '__main__':
    ...
    # soft_test_Y = predict(classifier_W_m, torch.from_numpy(test_X).float())  # sigmoid of forward pass
    ...
    for i in range(start_iterations, rest_iterations):
        ...
        # mapping_train_Y_new_test = predict(mapping_model, 0.1 * soft_test_Y + 0.9 * test_Y)
        mapping_model.eval()
        ...
        # mapping_train_Y_new_tensor_test = torch.from_numpy(mapping_train_Y_new_test).float()
        ...
        if use_al == True:
            batch_size = 40
            # test_X, test_Y, test_Y_rest
            # test_ds = torch.from_numpy(test_X).float(), mapping_train_Y_new_tensor_test, torch.from_numpy(test_Y).float(), torch.from_numpy(test_Y_rest).float()
